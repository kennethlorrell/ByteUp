...
const productNames = {
  cPUs: 'Процесори',
  gPUs: 'Відеокарти',
  coolers: 'Системи охолодження',
  motherboards: 'Материнські плати',
  hDDs: 'Жорсткі диски (HDD)',
  sSDs: 'Твердотілі накопичувачі (SSD)',
};
          ...
          <div className="menuSection">
            ...
            <div>
              <Link href="/" legacyBehavior>
                <p>Новинки та промоакції</p>
              </Link>
            </div>
            {productsListArr.map((item, idx) => {
              const spaced = item.replace('_', ' ');
              const listItemTitle = productNames[spaced] || spaced.charAt(0).toUpperCase() + spaced.slice(1);
